...
# TODO(developer)
with open("polygon_occupancies_config.json") as f1:
    inf = json.load(f1)
...
sql_engine = create_engine("mysql+pymysql://{user}:{pw}@{hostname}"
                        .format(user = inf["mysql"]["user"],
                                ...
                                pw = inf["mysql"]["passwd"]),pool_recycle=inf["mysql"]["pool_recycle"])
...
# if command_line_argument == "linecounti"
base_query = "insert into polygon_occupancy_dwell_records (polygon_id,tmp_occupancy,avg_occupancy,max_occupancy,dwell_time,create_time) values " 
